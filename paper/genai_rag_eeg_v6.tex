...
\textbf{Ours} & \textbf{'26} & \textbf{GenAI-RAG} & \textbf{Multi} & \textbf{99.3} & \textbf{Full} \\
\bottomrule
\end{tabular}
\end{table}
...
\noindent\textbf{Scoring Pipeline.} Each EEG segment traverses: (1)~515-feature extraction (band powers, Hjorth parameters, spectral entropy, stress ratios per channel); (2)~StandardScaler normalization (fitted on training fold only); (3)~ensemble prediction via soft voting across RandomForest ($n$=500), GradientBoosting ($n$=300), and SVM (RBF, $C$=10); (4)~probability-weighted class assignment ($\hat{y} = \arg\max_c \sum_m w_m P_m(c)$, equal weights $w_m=1/3$).

\noindent\textbf{Decision Logic.} Stress is predicted when $P(\text{stress}) > 0.5$. For EEGMAT, discriminability arises from strong alpha suppression (32.1\%, $d$=0.85) and beta enhancement (18.9\%, $d$=0.70) during mental arithmetic. For SAM-40, per-subject normalization removes inter-individual amplitude variability (contributing +12.5\% accuracy), while SMOTE corrects the 75:25 class imbalance (+8.3\%). The beta/alpha ratio emerges as the single strongest discriminative feature (SHAP rank \#1, mean $|\phi|$=0.42).

\noindent\textbf{Validation Timestamps.}
\begin{itemize}[leftmargin=*,nosep]
\item EEGMAT-Full training completed: 2026-01-03T10:39:41 UTC (results/full\_data\_results.json)
\item SAM-40 balanced training completed: 2026-01-04T20:48:27 UTC (results/sam40\_balanced\_v2\_results.json)
\item Paper compiled: 2026-03-19 (current version)
\item Repository: \url{https://github.com/PraveenAsthana123/stress1}
\end{itemize}
...
\noindent\textbf{Declaration of Originality.} The authors declare that this manuscript is original work and has not been published previously, nor is it under consideration for publication elsewhere. All results reported herein are derived from actual experimental training runs on real EEG datasets, verified against stored JSON result files and training logs.

\noindent\textbf{Conflict of Interest.} The authors declare no conflicts of interest.

\noindent\textbf{Ethics Statement.} Both datasets (EEGMAT and SAM-40) were collected under institutional review board (IRB) approval with written informed consent from all participants, as documented in their respective original publications.

\noindent\textbf{Data Availability.} EEGMAT is publicly available via PhysioNet (doi: 10.13026/C2JQ1P). SAM-40 is publicly available as described by Gupta et al.\ (2016). All preprocessing code, training scripts, trained model checkpoints, and evaluation results are available at the project repository.

\noindent\textbf{Code Availability.} Complete source code including all training scripts, preprocessing pipelines, figure generation, and evaluation protocols is publicly available at: \url{https://github.com/PraveenAsthana123/stress1}.

\noindent\textbf{Author Contributions.}
\begin{itemize}[leftmargin=*,nosep]
\item \textit{P. Asthana}: Conceptualization, methodology, software development, model training, data analysis, writing---original draft, project administration.
\item \textit{R.S. Lalawat}: Signal processing, EEG preprocessing, neurophysiological analysis, writing---review \& editing.
\item \textit{S.S. Gond}: Neurobiological validation, clinical interpretation, writing---review \& editing.
\end{itemize}

\noindent\textbf{Manuscript Timestamp.} Submitted: March 19, 2026. Last compiled: \today.
