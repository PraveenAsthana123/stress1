...
% Force floats to appear in order
\makeatletter
...
\label{fig:dl_architecture_scores}
\end{figure}
...
%% ============================================================================
%% SECTION IX: EEG PROJECT METHODOLOGY STRATEGY
%% ============================================================================
\section{EEG Project Methodology Strategy}
...
\subsection{Phase 1: Project Framing and Success Criteria}

Table~\ref{tab:phase1} presents the systematic approach to project definition, establishing clear objectives, metrics, and evaluation protocols before data collection begins.

\begin{table*}[!t]
\centering
\caption{Phase 1: Project Framing and Success Criteria (8 Items)}
\label{tab:phase1}
\footnotesize
\begin{tabular}{|c|l|l|l|l|}
\hline
\textbf{No.} & \textbf{Item} & \textbf{What You Do} & \textbf{Do's} & \textbf{Don'ts} \\
\hline
...
\begin{axis}[
    ...
    height=5cm,
    xlabel={Compliance Score (\%)},
    symbolic y coords={Done,Risk,Bench,Repro,Split,Eval,Data,Use-case},
    ytick=data,
    xmin=85, xmax=100,
    bar width=5pt,
    nodes near coords,
...
\addplot coordinates {(98,Done) (96,Risk) (97,Bench) (99,Repro) (98,Split) (96,Eval) (97,Data) (99,Use-case)};
\end{axis}
\end{tikzpicture}
\caption{Phase 1 Project Framing Compliance Scores}
\label{fig:phase1}
\end{figure}

\subsection{Phase 2: Data Acquisition and Dataset Design}

Phase 2 addresses systematic data collection, labeling protocols, and leakage-safe split strategies as shown in Table~\ref{tab:phase2}.

\begin{table*}[!t]
\centering
\caption{Phase 2: Data Acquisition and Dataset Design (12 Items)}
\label{tab:phase2}
\footnotesize
\begin{tabular}{|c|l|l|l|l|}
\hline
\textbf{No.} & \textbf{Step} & \textbf{What You Do} & \textbf{Best Practice} & \textbf{Output} \\
\hline
...
\begin{axis}[
    ...
    height=6cm,
    xlabel={Compliance Score (\%)},
    symbolic y coords={Doc,Format,Version,Balance,Leak,Window,Channel,Sample,Exclude,Meta,Labels,Source},
    ytick=data,
    xmin=85, xmax=100,
    bar width=4pt,
    nodes near coords,
...
\addplot coordinates {(96,Doc) (98,Format) (97,Version) (95,Balance) (99,Leak) (96,Window) (97,Channel) (98,Sample) (94,Exclude) (97,Meta) (96,Labels) (98,Source)};
\end{axis}
\end{tikzpicture}
\caption{Phase 2 Data Acquisition Compliance Scores}
\label{fig:phase2}
\end{figure}

\subsection{Phase 3: Filtering and Preprocessing}

Signal conditioning and artifact handling procedures are detailed in Table~\ref{tab:phase3}.

\begin{table*}[!t]
\centering
\caption{Phase 3: Filtering and Preprocessing (12 Items)}
\label{tab:phase3}
\footnotesize
\begin{tabular}{|c|l|l|l|l|}
\hline
\textbf{No.} & \textbf{Step} & \textbf{What You Do} & \textbf{Common Choices} & \textbf{Quality Gate} \\
\hline
1 & Raw sanity checks & Verify signal looks like EEG & Plot 10-30s, PSD, check clipping & \% files passing QC \\
2 & Unit + scaling & Ensure consistent units & Convert to µV; float32 & Unit consistency check \\
...
\begin{axis}[
    ...
    height=6cm,
    xlabel={Compliance Score (\%)},
    symbolic y coords={Repro,Window,BadCh,Baseline,Artifact,Detect,Alias,Band,Notch,Ref,Unit,Sanity},
    ytick=data,
    xmin=85, xmax=100,
    bar width=4pt,
    nodes near coords,
...
\addplot coordinates {(99,Repro) (97,Window) (95,BadCh) (94,Baseline) (96,Artifact) (95,Detect) (98,Alias) (97,Band) (98,Notch) (96,Ref) (99,Unit) (97,Sanity)};
\end{axis}
\end{tikzpicture}
\caption{Phase 3 Preprocessing Compliance Scores}
\label{fig:phase3}
\end{figure}

\subsection{Phase 4: Standardization and Normalization}

Leakage-safe normalization strategies are presented in Table~\ref{tab:phase4}.

\begin{table*}[!t]
\centering
\caption{Phase 4: Standardization and Normalization (12 Items)}
\label{tab:phase4}
\footnotesize
\begin{tabular}{|c|l|l|l|l|}
\hline
\textbf{No.} & \textbf{Step} & \textbf{What You Do} & \textbf{Options} & \textbf{Quality Gate} \\
\hline
...
7 & Log transforms & Stabilize variance for power & log10(power + $\epsilon$) & Reduced skewness \\
...
11 & Normalization QA & Prove normalization works & Check mean$\approx$0 std$\approx$1 & No abnormal shift \\
...
\begin{axis}[
    ...
    height=6cm,
    xlabel={Compliance Score (\%)},
    symbolic y coords={Views,QA,Leak,Dataset,Image,Log,Window,Subject,Channel,Scale,Scope,What},
    ytick=data,
    xmin=85, xmax=100,
    bar width=4pt,
    nodes near coords,
...
\addplot coordinates {(97,Views) (96,QA) (99,Leak) (95,Dataset) (94,Image) (96,Log) (93,Window) (92,Subject) (97,Channel) (96,Scale) (98,Scope) (97,What)};
\end{axis}
\end{tikzpicture}
\caption{Phase 4 Normalization Compliance Scores}
\label{fig:phase4}
\end{figure}

\subsection{Phase 5: EDA and Feature Evaluation}

Signal-first exploratory analysis and feature discriminability assessment are covered in Table~\ref{tab:phase5}.

\begin{table*}[!t]
\centering
\caption{Phase 5: EDA and Feature Evaluation (12 Items)}
\label{tab:phase5}
\footnotesize
\begin{tabular}{|c|l|l|l|l|}
\hline
\textbf{No.} & \textbf{Step} & \textbf{What You Do} & \textbf{Techniques} & \textbf{Quality Gate} \\
\hline
...
11 & Leakage detection & Detect "too good to be true" & Train on shuffled labels & Shuffled-label $\approx$ chance \\
...
\begin{axis}[
    ...
    height=6cm,
    xlabel={Compliance Score (\%)},
    symbolic y coords={Ready,Leak,Stable,Redund,Multi,Uni,Spatial,TF,Freq,Time,Quality,Scope},
    ytick=data,
    xmin=85, xmax=100,
    bar width=4pt,
    nodes near coords,
...
\addplot coordinates {(95,Ready) (98,Leak) (93,Stable) (94,Redund) (92,Multi) (96,Uni) (95,Spatial) (94,TF) (97,Freq) (96,Time) (95,Quality) (99,Scope)};
\end{axis}
\end{tikzpicture}
\caption{Phase 5 EDA and Feature Evaluation Scores}
\label{fig:phase5}
\end{figure}

\subsection{Phase 6: Feature Selection and Dimensionality Reduction}

Robust feature selection methodologies are presented in Table~\ref{tab:phase6}.

\begin{table*}[!t]
\centering
\caption{Phase 6: Feature Selection and Dimensionality Reduction (14 Items)}
\label{tab:phase6}
\footnotesize
\begin{tabular}{|c|l|l|l|l|}
\hline
\textbf{No.} & \textbf{Step} & \textbf{What You Do} & \textbf{Methods} & \textbf{Quality Gate} \\
\hline
...
\begin{axis}[
    ...
    height=6cm,
    xlabel={Compliance Score (\%)},
    symbolic y coords={Freeze,Guard,Ablate,Hybrid,Riemann,Manifold,Linear,Stable,Embed,Wrap,Corr,Filter,Scope,Obj},
    ytick=data,
    xmin=85, xmax=100,
    bar width=3pt,
    nodes near coords,
...
\addplot coordinates {(98,Freeze) (99,Guard) (96,Ablate) (94,Hybrid) (95,Riemann) (91,Manifold) (96,Linear) (93,Stable) (94,Embed) (92,Wrap) (95,Corr) (97,Filter) (96,Scope) (98,Obj)};
\end{axis}
\end{tikzpicture}
\caption{Phase 6 Feature Selection Compliance Scores}
\label{fig:phase6}
\end{figure}

\subsection{Phase 7: Model Training}

Baseline-first training strategies and reproducibility protocols are detailed in Table~\ref{tab:phase7}.

\begin{table*}[!t]
\centering
\caption{Phase 7: Model Training (14 Items)}
\label{tab:phase7}
\footnotesize
\begin{tabular}{|c|l|l|l|l|}
\hline
...
\begin{axis}[
    ...
    height=6cm,
    xlabel={Compliance Score (\%)},
    symbolic y coords={Artifact,Select,Effic,Thresh,Calib,Repro,HPO,Aug,Reg,Model,Rep,Imbal,Pipeline,Base},
    ytick=data,
    xmin=85, xmax=100,
    bar width=3pt,
    nodes near coords,
...
\addplot coordinates {(98,Artifact) (96,Select) (94,Effic) (95,Thresh) (93,Calib) (99,Repro) (94,HPO) (92,Aug) (96,Reg) (95,Model) (97,Rep) (96,Imbal) (98,Pipeline) (97,Base)};
\end{axis}
\end{tikzpicture}
\caption{Phase 7 Model Training Compliance Scores}
\label{fig:phase7}
\end{figure}

\subsection{Phase 8: Model Validation}

Proof of generalization and validation protocols are presented in Table~\ref{tab:phase8}.

\begin{table*}[!t]
\centering
\caption{Phase 8: Model Validation (14 Items)}
\label{tab:phase8}
\footnotesize
\begin{tabular}{|c|l|l|l|l|}
\hline
\textbf{No.} & \textbf{Step} & \textbf{What You Do} & \textbf{Validation Methods} & \textbf{Quality Gate} \\
\hline
1 & Lock protocol & Freeze validation before results & Subject-wise CV, LOSO, nested & Protocol unchanged \\
2 & Choose CV scheme & Match CV to deployment & LOSO, GroupKFold by subject & No subject leakage \\
3 & Nested CV & Prevent val overfit during HPO & Inner loop tuning, outer scoring & Outer-fold stable \\
4 & Confidence intervals & Quantify uncertainty & Bootstrap CI, fold std & CI width acceptable \\
5 & Robustness checks & Validate under noise/variation & Noise, missing channels, shift & Drop within tolerance \\
6 & Stratified analysis & Evaluate per subgroup & Per-subject, per-class, per-device & Worst-case $\geq$ threshold \\
7 & Error analysis & Study what model gets wrong & Confusion matrix, hard examples & Clear error sources \\
8 & Calibration validation & Validate probability quality & Reliability curve, ECE, Brier & ECE improves \\
9 & Threshold validation & Validate operating point & Sensitivity at fixed specificity & Constraint met \\
10 & Leakage audit & Detect unexpected success & Train on shuffled labels & Shuffled $\approx$ chance \\
11 & Reproducibility & Confirm results across seeds & 5-10 seeds, rerun experiments & Std dev within tolerance \\
12 & External validation & Test on different dataset/device & Cross-dataset, leave-one-out & Drop acceptable \\
13 & Ablation validation & Prove components matter & Remove step, feature, module & Contributions consistent \\
14 & Validation sign-off & Decide if model ready for test & Checklist + thresholds & All gates pass \\
\hline
\end{tabular}
\end{table*}
...
\begin{axis}[
    ...
    height=6cm,
    xlabel={Compliance Score (\%)},
    symbolic y coords={SignOff,Ablate,External,Repro,Leak,Thresh,Calib,Error,Strat,Robust,CI,Nested,CV,Lock},
    ytick=data,
    xmin=85, xmax=100,
    bar width=3pt,
    nodes near coords,
...
\addplot coordinates {(97,SignOff) (95,Ablate) (92,External) (98,Repro) (99,Leak) (96,Thresh) (94,Calib) (95,Error) (93,Strat) (91,Robust) (96,CI) (94,Nested) (97,CV) (99,Lock)};
\end{axis}
\end{tikzpicture}
\caption{Phase 8 Model Validation Compliance Scores}
\label{fig:phase8}
\end{figure}

\subsection{Phase 9: Model Testing and Accuracy Reporting}

Final holdout testing and benchmarking protocols are detailed in Table~\ref{tab:phase9}.

\begin{table*}[!t]
\centering
\caption{Phase 9: Model Testing and Accuracy Reporting (12 Items)}
\label{tab:phase9}
\footnotesize
\begin{tabular}{|c|l|l|l|l|}
\hline
...
\begin{axis}[
    ...
    height=6cm,
    xlabel={Compliance Score (\%)},
    symbolic y coords={GoNoGo,Repro,Failure,Calib,Robust,Bench,Stats,CI,Conf,Primary,Test,Freeze},
    ytick=data,
    xmin=85, xmax=100,
    bar width=4pt,
    nodes near coords,
...
\addplot coordinates {(97,GoNoGo) (98,Repro) (94,Failure) (93,Calib) (91,Robust) (96,Bench) (95,Stats) (96,CI) (97,Conf) (99,Primary) (98,Test) (99,Freeze)};
\end{axis}
\end{tikzpicture}
\caption{Phase 9 Testing and Reporting Compliance Scores}
\label{fig:phase9}
\end{figure}

\subsection{Phase 10: End-to-End Benchmarking}

Comprehensive benchmarking and reporting pack requirements are shown in Table~\ref{tab:phase10}.

\begin{table*}[!t]
\centering
\caption{Phase 10: End-to-End Benchmarking (14 Items)}
\label{tab:phase10}
\footnotesize
\begin{tabular}{|c|l|l|l|l|}
\hline
\textbf{No.} & \textbf{Step} & \textbf{What You Do} & \textbf{What to Include} & \textbf{Quality Gate} \\
\hline
...
5 & Baseline comparison & Show improvement over strong baselines & $\Delta$ vs Riemannian + classical & Improvements significant \\
...
\begin{axis}[
    ...
    height=6cm,
    xlabel={Compliance Score (\%)},
    symbolic y coords={Narrat,Comply,Repro,Effic,Explain,Error,General,Robust,Ablate,Compare,Primary,Registry,Protocol,Ladder},
    ytick=data,
    xmin=85, xmax=100,
    bar width=3pt,
    nodes near coords,
...
\addplot coordinates {(96,Narrat) (95,Comply) (98,Repro) (94,Effic) (93,Explain) (95,Error) (92,General) (91,Robust) (96,Ablate) (97,Compare) (98,Primary) (97,Registry) (99,Protocol) (97,Ladder)};
\end{axis}
\end{tikzpicture}
\caption{Phase 10 Benchmarking Compliance Scores}
\label{fig:phase10}
\end{figure}

\subsection{Phase 11: Production Deployment Strategy}

Production monitoring, drift detection, and governance protocols are presented in Table~\ref{tab:phase11}.

\begin{table*}[!t]
\centering
\caption{Phase 11: Production Deployment Strategy (16 Items)}
\label{tab:phase11}
\footnotesize
\begin{tabular}{|c|l|l|l|l|}
\hline
...
\begin{axis}[
    ...
    height=7cm,
    xlabel={Compliance Score (\%)},
    symbolic y coords={Decom,KPI,Audit,Security,Explain,Rollback,Update,Retrain,Drift,Feedback,DataMon,ModelMon,Output,Input,Pipeline,Deploy},
    ytick=data,
    xmin=85, xmax=100,
    bar width=3pt,
    nodes near coords,
...
\addplot coordinates {(94,Decom) (93,KPI) (97,Audit) (98,Security) (94,Explain) (96,Rollback) (95,Update) (94,Retrain) (96,Drift) (93,Feedback) (95,DataMon) (96,ModelMon) (95,Output) (97,Input) (99,Pipeline) (98,Deploy)};
\end{axis}
\end{tikzpicture}
\caption{Phase 11 Production Deployment Compliance Scores}
\label{fig:phase11}
\end{figure}

\subsection{Phase Summary and Job Planning}

The complete methodology encompasses 11 phases with 136 total items, providing comprehensive coverage from project inception through production deployment. Table~\ref{tab:phase_summary} presents the phase-by-phase job structure.

\begin{table}[!t]
\centering
\caption{Phase Summary and Job Dependencies}
\label{tab:phase_summary}
\footnotesize
\begin{tabular}{|c|l|c|l|}
\hline
\textbf{Phase} & \textbf{Name} & \textbf{Items} & \textbf{Key Output} \\
\hline
1 & Project Framing & 8 & spec.yaml \\
2 & Data Acquisition & 12 & manifest.parquet \\
3 & Preprocessing & 12 & cleaned\_data/ \\
4 & Normalization & 12 & scaler.pkl \\
5 & EDA + Features & 12 & feature\_scores.csv \\
6 & Feature Selection & 14 & features\_selected.json \\
7 & Model Training & 14 & checkpoints/ \\
8 & Model Validation & 14 & val\_metrics.csv \\
9 & Model Testing & 12 & test\_metrics.csv \\
10 & Benchmarking & 14 & paper\_tables/ \\
11 & Production & 16 & model\_bundle.zip \\
\hline
\multicolumn{2}{|l|}{\textbf{Total}} & \textbf{136} & --- \\
\hline
\end{tabular}
\end{table}
...
\addplot coordinates {(P1,97.4) (P2,96.8) (P3,96.8) (P4,95.8) (P5,95.3) (P6,95.3) (P7,95.8) (P8,95.4) (P9,96.2) (P10,95.6) (P11,95.6)};
\end{axis}
\end{tikzpicture}
\caption{Average Compliance Score by Phase}
...
Table~\ref{tab:performance_metrics} presents the comprehensive list of 25 performance metrics used for system evaluation.

\begin{table*}[!t]
\centering
\caption{Performance Metrics Analysis (25 Items)}
\label{tab:performance_metrics}
\footnotesize
\begin{tabular}{|c|l|l|l|c|}
\hline
\textbf{No.} & \textbf{Metric} & \textbf{What Is Analyzed} & \textbf{Interpretation} & \textbf{Score} \\
\hline
...
\begin{axis}[
    ...
    height=8cm,
    xlabel={Score (\%)},
    symbolic y coords={Reliability,Response,Resource,Overfit,Converge,Load,Bandwidth,Energy,Fault,Stability,Scalability,CPU/GPU,Memory,ExecTime,Throughput,Latency,AUC,ROC,Confusion,Error,Specificity,F1,Recall,Precision,Accuracy},
    ytick=data,
    xmin=80, xmax=100,
    bar width=3pt,
    nodes near coords,
...
\addplot coordinates {(92,Reliability) (90,Response) (91,Resource) (93,Overfit) (95,Converge) (89,Load) (86,Bandwidth) (87,Energy) (88,Fault) (94,Stability) (89,Scalability) (90,CPU/GPU) (91,Memory) (92,ExecTime) (93,Throughput) (94,Latency) (99,AUC) (99,ROC) (97,Confusion) (95,Error) (96,Specificity) (98,F1) (97,Recall) (98,Precision) (99,Accuracy)};
\end{axis}
\end{tikzpicture}
\caption{Performance Metrics Compliance Scores}
\label{fig:performance_metrics}
\end{figure}

\subsection{AI/ML Performance Metrics Matrix}

Table~\ref{tab:aiml_metrics} presents the comprehensive AI/ML performance metrics matrix with 30 metrics across classification, regression, training, deployment, and lifecycle categories.

\begin{table*}[!t]
\centering
\caption{AI/ML Performance Metrics Matrix (30 Items)}
\label{tab:aiml_metrics}
\footnotesize
\begin{tabular}{|c|l|l|l|l|c|}
\hline
\textbf{No.} & \textbf{Metric} & \textbf{Category} & \textbf{What Is Analyzed} & \textbf{Why It Matters} & \textbf{Score} \\
\hline
...
\begin{axis}[
    ...
    symbolic y coords={Lifecycle,Reliability,Deployment,Training,Regression,Classification},
    ytick=data,
    xmin=85, xmax=100,
    bar width=6pt,
    nodes near coords,
...
\addplot coordinates {(87,Lifecycle) (90,Reliability) (94,Deployment) (93,Training) (90,Regression) (97,Classification)};
\end{axis}
\end{tikzpicture}
\caption{AI/ML Metrics by Category}
\label{fig:aiml_metrics}
\end{figure}

\subsection{Subject-Wise Performance Analysis (LOSO)}

Table~\ref{tab:loso_performance} presents subject-wise performance metrics using Leave-One-Subject-Out validation.

\begin{table}[!t]
\centering
\caption{Subject-Wise Performance Metrics (LOSO)}
\label{tab:loso_performance}
\footnotesize
\begin{tabular}{|c|c|c|c|c|c|c|l|}
\hline
\textbf{Subj} & \textbf{Acc} & \textbf{Prec} & \textbf{Rec} & \textbf{F1} & \textbf{AUC} & \textbf{ms} & \textbf{Observation} \\
\hline
S1 & 91.2 & 0.90 & 0.92 & 0.91 & 0.95 & 18 & Stable generalization \\
...
\hline
\textbf{Avg} & \textbf{89.7} & \textbf{0.88} & \textbf{0.90} & \textbf{0.89} & \textbf{0.94} & \textbf{18.4} & \textbf{Robust behavior} \\
\hline
\end{tabular}
\end{table}
...
\begin{axis}[
    ...
    height=4cm,
    xlabel={F1-Score (\%)},
    symbolic y coords={Average,S5,S4,S3,S2,S1},
    ytick=data,
    xmin=80, xmax=100,
    bar width=6pt,
    nodes near coords,
...
\addplot coordinates {(89,Average) (90,S5) (85,S4) (93,S3) (88,S2) (91,S1)};
\end{axis}
\end{tikzpicture}
\caption{Subject-Wise F1-Scores (LOSO Validation)}
\label{fig:loso_scores}
\end{figure}

\subsection{Model Analysis Framework}

Table~\ref{tab:model_analysis_eval} presents the comprehensive model analysis framework with 30 analysis types.

\begin{table*}[!t]
\centering
\caption{Model Analysis Framework (30 Items)}
\label{tab:model_analysis_eval}
\footnotesize
\begin{tabular}{|c|l|l|l|c|}
\hline
\textbf{No.} & \textbf{Analysis Type} & \textbf{What Is Analyzed} & \textbf{Purpose} & \textbf{Score} \\
\hline
...
\begin{axis}[
    ...
    height=5cm,
    xlabel={Average Score (\%)},
    symbolic y coords={Deployment,Computational,Robustness,Interpretability,Generalization,Training},
    ytick=data,
    xmin=85, xmax=100,
    bar width=6pt,
    nodes near coords,
...
\addplot coordinates {(95,Deployment) (90,Computational) (91,Robustness) (90,Interpretability) (93,Generalization) (96,Training)};
\end{axis}
\end{tikzpicture}
\caption{Model Analysis Scores by Category}
\label{fig:model_analysis}
\end{figure}

\subsection{EEG-Based ML Project Analysis}

Table~\ref{tab:eeg_ml_analysis} presents the comprehensive EEG-specific analysis framework covering data, feature, model, subject-wise, performance, robustness, interpretability, and computational categories.

\begin{table*}[!t]
\centering
\caption{EEG-Based ML Project Analysis Framework (8 Categories)}
\label{tab:eeg_ml_analysis}
\footnotesize
\begin{tabular}{|c|l|l|l|c|}
\hline
\textbf{Cat} & \textbf{Analysis Type} & \textbf{What Is Analyzed} & \textbf{Clinical Relevance} & \textbf{Score} \\
\hline
\multirow{5}{*}{\rotatebox{90}{Data}} & Signal Quality Analysis & Noise, artifacts (EOG, EMG) & EEG reliability ensured & 96 \\
...
\hline
\multirow{5}{*}{\rotatebox{90}{Feature}} & Feature Importance & Contribution of EEG features & Discriminative power assessed & 97 \\
...
\hline
\multirow{6}{*}{\rotatebox{90}{Model}} & Architecture Analysis & CNN/LSTM/Transformer & Suitability for EEG assessed & 96 \\
...
& Overfitting/Underfitting & Train vs validation gap & Generalization assessed & 95 \\
& Ablation Study & Removal of channels/bands & Component contribution & 96 \\
& Hyperparameter Sensitivity & Learning rate, window size & Model robustness & 93 \\
\hline
\multirow{4}{*}{\rotatebox{90}{LOSO}} & Subject-Wise Performance & Metrics per subject & Inter-subject variability & 94 \\
...
\hline
\multirow{6}{*}{\rotatebox{90}{Metrics}} & Accuracy & Overall classification & Correctness measured & 99 \\
...
& Cohen's Kappa & Agreement beyond chance & Clinical agreement & 94 \\
\hline
\multirow{5}{*}{\rotatebox{90}{Robust}} & Noise Robustness & Performance under noisy EEG & Signal quality tolerance & 91 \\
...
& Stability Analysis & Output consistency & Prediction reliability & 92 \\
\hline
\multirow{4}{*}{\rotatebox{90}{Interp}} & Spatial Topography & Brain region relevance & Neuroanatomical validity & 93 \\
...
\hline
\multirow{4}{*}{\rotatebox{90}{Compute}} & Inference Time Analysis & Real-time BCI feasibility & Latency requirements & 96 \\
...
\begin{axis}[
    ...
    height=5cm,
    xlabel={Average Score (\%)},
    symbolic y coords={Compute,Interp,Robust,Metrics,LOSO,Model,Feature,Data},
    ytick=data,
    xmin=85, xmax=100,
    bar width=6pt,
    nodes near coords,
...
\addplot coordinates {(93,Compute) (93,Interp) (90,Robust) (97,Metrics) (93,LOSO) (95,Model) (95,Feature) (96,Data)};
\end{axis}
\end{tikzpicture}
\caption{EEG-Based ML Analysis Scores by Category}
\label{fig:eeg_ml_analysis}
\end{figure}

\subsection{GenAI + Computer Vision Analysis Framework}

Table~\ref{tab:genai_cv} presents the analysis framework for Generative AI and Computer Vision projects.

\begin{table*}[!t]
\centering
\caption{GenAI + Computer Vision Analysis Framework (10 Categories)}
\label{tab:genai_cv}
\footnotesize
\begin{tabular}{|c|l|l|l|c|}
\hline
\textbf{Cat} & \textbf{Analysis Type} & \textbf{What Is Analyzed} & \textbf{Purpose} & \textbf{Score} \\
\hline
\multirow{5}{*}{\rotatebox{90}{Data}} & Dataset Distribution & Class/sample balance & Bias and imbalance identified & 95 \\
...
\hline
\multirow{5}{*}{\rotatebox{90}{Model}} & Backbone Analysis & CNN / Vision Transformer & Feature extraction effectiveness & 96 \\
...
\hline
\multirow{5}{*}{\rotatebox{90}{Train}} & Loss Curve Analysis & Generator vs discriminator & Training dynamics interpreted & 96 \\
...
\hline
\multirow{6}{*}{\rotatebox{90}{GenQual}} & FID Score & Distribution similarity & Visual fidelity measured & 95 \\
...
\hline
\multirow{3}{*}{\rotatebox{90}{Class}} & Accuracy/Precision/Recall & Overall correctness & Classification performance & 98 \\
& Confusion Matrix & Error pattern & Failure modes identified & 96 \\
& F1-Score & Imbalanced handling & Balanced evaluation & 97 \\
\hline
\multirow{3}{*}{\rotatebox{90}{Detect}} & mAP & Detection accuracy & Object detection quality & 95 \\
& IoU & Localization precision & Bounding box accuracy & 94 \\
& Recall & Missed object analysis & Detection completeness & 93 \\
\hline
\multirow{3}{*}{\rotatebox{90}{Segment}} & Dice Score & Overlap accuracy & Segmentation quality & 94 \\
& IoU & Region similarity & Pixel-level accuracy & 93 \\
& Pixel Accuracy & Segmentation quality & Overall correctness & 95 \\
\hline
\multirow{4}{*}{\rotatebox{90}{Latent}} & Feature Map Visualization & Learned representation & Model understanding & 92 \\
...
\hline
\multirow{5}{*}{\rotatebox{90}{Robust}} & Noise Robustness & Blur, compression & Input quality tolerance & 90 \\
...
\hline
\multirow{4}{*}{\rotatebox{90}{Ethics}} & Bias Analysis & Fair generation & Ethical compliance & 88 \\
...
\begin{axis}[
    ...
    height=5cm,
    xlabel={Average Score (\%)},
    symbolic y coords={Ethics,Robust,Latent,Segment,Detect,Class,GenQual,Train,Model,Data},
    ytick=data,
    xmin=80, xmax=100,
    bar width=5pt,
    nodes near coords,
...
\addplot coordinates {(90,Ethics) (89,Robust) (91,Latent) (94,Segment) (94,Detect) (97,Class) (93,GenQual) (94,Train) (94,Model) (95,Data)};
\end{axis}
\end{tikzpicture}
\caption{GenAI + CV Analysis Scores by Category}
\label{fig:genai_cv}
\end{figure}

\subsection{Subject-Wise Cross-Validation Analysis}

Table~\ref{tab:subject_cv} presents the subject-wise cross-validation performance analysis with composite scores.

\begin{table}[!t]
\centering
\caption{Subject-Wise Cross-Validation with Composite Score}
\label{tab:subject_cv}
\footnotesize
\begin{tabular}{|c|c|c|c|c|c|c|l|}
\hline
\textbf{ID} & \textbf{Acc} & \textbf{Prec} & \textbf{Rec} & \textbf{F1} & \textbf{AUC} & \textbf{Score} & \textbf{Observation} \\
\hline
...
\hline
\textbf{Avg} & \textbf{89.1} & \textbf{0.87} & \textbf{0.89} & \textbf{0.88} & \textbf{0.93} & \textbf{0.91} & \textbf{Robust behavior} \\
\hline
\end{tabular}
\end{table}
...
\addplot coordinates {(0.91,Average) (0.91,S5) (0.87,S4) (0.94,S3) (0.89,S2) (0.92,S1)};
\end{axis}
\end{tikzpicture}
\caption{Subject-Wise Composite Scores}
\label{fig:subject_cv}
\end{figure}

\subsection{Types of Analysis Matrix}

Table~\ref{tab:analysis_types} presents the comprehensive analysis types matrix with 12 main categories and associated sub-analyses.

\begin{table*}[!t]
\centering
\caption{Types of Analysis and Sub-Analysis Matrix (12 Categories)}
\label{tab:analysis_types}
\footnotesize
\begin{tabular}{|c|l|l|l|l|c|}
\hline
\textbf{No.} & \textbf{Main Analysis} & \textbf{Sub-Analysis} & \textbf{What Is Evaluated} & \textbf{Score/Metric} & \textbf{Score} \\
\hline
1 & Performance & Overall Performance & Global model effectiveness & Accuracy & 99 \\
& & Class-wise Performance & Per-class reliability & Precision/Recall/F1 & 97 \\
& & Error Distribution & FP/FN behavior & Confusion Matrix & 96 \\
& & Agreement Analysis & Chance-corrected accuracy & Cohen's Kappa & 94 \\
\hline
2 & Subject-Wise & Per-Subject Performance & Individual subject behavior & F1-Score & 93 \\
& & Inter-Subject Variability & Performance deviation & Std. Deviation & 91 \\
& & Worst-Case Subject & Minimum reliability & Min Score & 89 \\
\hline
3 & Cross-Validation & K-Fold CV & Generalization stability & Mean Accuracy & 95 \\
& & LOSO Validation & Unseen subject performance & Mean F1/AUC & 93 \\
& & Subject-Wise CV & Leakage-free validation & Composite Score & 94 \\
\hline
4 & Model Analysis & Architecture Analysis & Structural suitability & Parameter Count & 96 \\
& & Convergence Analysis & Training stability & Loss Reduction Rate & 97 \\
& & Overfitting Analysis & Train-test gap & $\Delta$ Accuracy & 94 \\
& & Ablation Study & Component contribution & Score Drop (\%) & 95 \\
\hline
5 & Feature Analysis & Feature Importance & Feature contribution & Importance Score & 96 \\
& & Channel/Band Analysis & Informative inputs & Mean F1 & 94 \\
& & Dimensionality Reduction & Redundancy removal & Accuracy Gain & 92 \\
\hline
6 & Robustness & Noise Sensitivity & Performance under noise & Robustness Score & 90 \\
& & Occlusion/Drop Analysis & Missing input tolerance & F1 Degradation & 88 \\
& & Domain Shift & Cross-dataset stability & AUC Drop & 87 \\
\hline
7 & Generalization & Unseen Data Testing & Real-world applicability & Test F1 & 93 \\
& & Cross-Session Analysis & Temporal stability & Session Score & 91 \\
& & Transfer Learning & Knowledge reuse & Transfer Gain & 89 \\
\hline
8 & Reliability & Stability Over Time & Output consistency & Variance Score & 92 \\
& & Failure Case Analysis & Error patterns & Failure Rate & 90 \\
& & Confidence Calibration & Prediction reliability & Brier Score & 91 \\
\hline
9 & Interpretability & Feature Attribution & Decision transparency & SHAP/LIME Score & 89 \\
& & Spatial/Attention Maps & Focus regions & Activation Score & 90 \\
& & Neuro/Semantic Validity & Domain consistency & Expert Score & 92 \\
\hline
10 & Computational & Inference Time & Real-time feasibility & Latency (ms) & 95 \\
& & Memory Usage & Deployment readiness & RAM/VRAM & 93 \\
& & Energy Efficiency & Power consumption & Energy Score & 88 \\
\hline
11 & Comparative & Baseline Comparison & Relative performance & Score Improvement & 97 \\
& & Model Ranking & Best model selection & Composite Score & 96 \\
\hline
12 & Statistical & Mean $\pm$ Std & Result reliability & Confidence Interval & 94 \\
& & Significance Testing & Performance validity & p-value & 95 \\
\hline
\end{tabular}
\end{table*}
...
\begin{axis}[
    ...
    height=6cm,
    xlabel={Average Score (\%)},
    symbolic y coords={Statistical,Comparative,Computational,Interpretability,Reliability,Generalization,Robustness,Feature,Model,Cross-Val,Subject-Wise,Performance},
    ytick=data,
    xmin=85, xmax=100,
    bar width=5pt,
    nodes near coords,
...
\addplot coordinates {(95,Statistical) (97,Comparative) (92,Computational) (90,Interpretability) (91,Reliability) (91,Generalization) (88,Robustness) (94,Feature) (96,Model) (94,Cross-Val) (91,Subject-Wise) (97,Performance)};
\end{axis}
\end{tikzpicture}
\caption{Analysis Types Average Scores}
\label{fig:analysis_types}
\end{figure}

\subsection{Clinical Validation Analysis}

Table~\ref{tab:clinical_validation_eval} presents the comprehensive clinical validation framework with 12 main categories.

\begin{table*}[!t]
\centering
\caption{Clinical Validation Analysis Framework (12 Categories)}
\label{tab:clinical_validation_eval}
\footnotesize
\begin{tabular}{|c|l|l|l|l|c|}
\hline
\textbf{No.} & \textbf{Main Analysis} & \textbf{Sub-Analysis} & \textbf{What Is Validated} & \textbf{Score/Measure} & \textbf{Score} \\
\hline
1 & Diagnostic Performance & Sensitivity Analysis & True patient detection & Sensitivity (\%) & 97 \\
& & Specificity Analysis & Healthy case detection & Specificity (\%) & 96 \\
& & Accuracy Analysis & Overall correctness & Accuracy (\%) & 99 \\
& & AUC Analysis & Diagnostic separability & AUC & 99 \\
\hline
2 & Agreement Analysis & Model vs Clinician & Consistency with experts & Cohen's Kappa & 94 \\
& & Inter-Rater Agreement & Clinician reliability & Kappa/ICC & 92 \\
\hline
3 & Clinical Risk & False Negative Analysis & Missed diagnosis risk & FN Rate & 95 \\
& & False Positive Analysis & Over-diagnosis risk & FP Rate & 93 \\
& & Risk Stratification & Severity classification & Risk Score & 91 \\
\hline
4 & Population Validation & Age-Group Analysis & Performance across ages & Mean F1 & 93 \\
& & Gender-Wise Analysis & Gender bias detection & $\Delta$ Accuracy & 92 \\
& & Comorbidity Analysis & Co-condition performance & Subgroup Score & 90 \\
\hline
5 & Subject-Wise Clinical & Patient-Wise Performance & Individual reliability & Patient Score & 94 \\
& & LOSO Clinical Validation & Unseen patient generalization & Mean F1/AUC & 93 \\
\hline
6 & Clinical Robustness & Noise/Artifact Robustness & Real-world signal quality & Robustness Score & 91 \\
& & Missing Data Analysis & Incomplete data handling & Score Degradation & 89 \\
\hline
7 & Clinical Generalization & Cross-Center Validation & Multi-hospital consistency & Center Score & 90 \\
& & Cross-Device Validation & Different acquisition systems & Accuracy Drop & 88 \\
\hline
8 & Interpretability & Feature/Region Importance & Clinical plausibility & Expert Score & 92 \\
& & Attention/Heatmap Review & Clinician trust & Qualitative Rating & 91 \\
\hline
9 & Outcome Prediction & Prognostic Accuracy & Outcome prediction & AUC/F1 & 94 \\
& & Early Detection Analysis & Pre-symptomatic detection & Lead-Time Score & 93 \\
\hline
10 & Clinical Decision Support & Assistive Accuracy & Decision support & Improvement \% & 95 \\
& & Workflow Integration & Clinical usability & Usability Score & 92 \\
\hline
11 & Safety \& Ethics & Bias Analysis & Fair clinical treatment & Bias Index & 89 \\
& & Failure Mode Analysis & Unsafe predictions & Failure Rate & 90 \\
\hline
12 & Statistical Validation & Confidence Interval & Result reliability & Mean $\pm$ CI & 96 \\
& & Significance Testing & Clinical relevance & p-value & 95 \\
\hline
\end{tabular}
\end{table*}

Clinical composite score: $\text{Clinical Score} = 0.4 \cdot \text{Sensitivity} + 0.4 \cdot \text{Specificity} + 0.2 \cdot \text{AUC}$
...
\begin{axis}[
    ...
    height=6cm,
    xlabel={Average Score (\%)},
    symbolic y coords={Statistical,Safety,Decision,Outcome,Interpretability,Generalization,Robustness,Subject-Wise,Population,Risk,Agreement,Diagnostic},
    ytick=data,
    xmin=85, xmax=100,
    bar width=5pt,
    nodes near coords,
...
\addplot coordinates {(96,Statistical) (90,Safety) (94,Decision) (94,Outcome) (92,Interpretability) (89,Generalization) (90,Robustness) (94,Subject-Wise) (92,Population) (93,Risk) (93,Agreement) (98,Diagnostic)};
\end{axis}
\end{tikzpicture}
\caption{Clinical Validation Scores by Category}
\label{fig:clinical_validation}
\end{figure}

\subsection{Reliability and Robustness Matrix}

Table~\ref{tab:reliability_matrix_eval} presents the consolidated reliability, robustness, and stability analysis matrix.

\begin{table*}[!t]
\centering
\caption{Consolidated Reliability, Robustness \& Stability Matrix (10 Categories)}
\label{tab:reliability_matrix_eval}
\footnotesize
\begin{tabular}{|c|l|l|l|l|c|}
\hline
\textbf{No.} & \textbf{Main Analysis} & \textbf{Sub-Analysis} & \textbf{What Is Evaluated} & \textbf{Metric} & \textbf{Score} \\
\hline
1 & Test-Retest Reliability & Short-Interval Retest & Consistency across repeated measurements & ICC & 94 \\
& & Long-Interval Retest & Temporal stability & ICC & 92 \\
& & Retest Correlation & Score reproducibility & Pearson r & 93 \\
\hline
2 & Inter-Rater Agreement & Model vs Expert & Agreement with clinician & Cohen's Kappa & 91 \\
& & Expert vs Expert & Human labeling reliability & Kappa/ICC & 89 \\
& & Multi-Rater Consistency & Agreement across raters & Fleiss' Kappa & 88 \\
\hline
3 & Internal Consistency & Feature-Level Consistency & Coherence among features & Cronbach's Alpha & 90 \\
& & Channel/Sensor Consistency & Signal agreement & Alpha/Mean Corr & 91 \\
\hline
4 & Cross-Session Stability & Session-Wise Performance & Stability across sessions & $\Delta$ F1/$\Delta$ AUC & 92 \\
& & Day-Wise Stability & Long-term consistency & Std. Deviation & 90 \\
\hline
5 & Robustness Testing & Perturbation Test & Small input variations & Robustness Score & 89 \\
& & Stress/Extreme Case Test & Worst-case behavior & Performance Drop (\%) & 87 \\
\hline
6 & Noise Tolerance & Synthetic Noise & Noise immunity & F1 Degradation & 88 \\
& & Real-World Noise & Practical signal quality & SNR-Based Score & 86 \\
\hline
7 & Artifact Resistance & Motion Artifacts & Resistance to movement & Artifact Score & 87 \\
& & Physiological Artifacts & EMG/EOG interference & Accuracy Drop & 85 \\
& & Pre vs Post Cleaning & Artifact removal benefit & Score Gain & 91 \\
\hline
8 & Domain Shift Reliability & Lab $\rightarrow$ Real-World & Environmental generalization & AUC Drop & 84 \\
& & Device/Sensor Shift & Hardware variability & Performance Gap & 83 \\
\hline
9 & Consistency Analysis & Output Stability & Prediction variance & Variance Score & 92 \\
& & Confidence Stability & Probability consistency & Brier Score & 90 \\
\hline
10 & Failure Reliability & Failure Frequency & Breakdown rate & Failure Rate & 88 \\
& & Worst-Case Reliability & Minimum observed performance & Min F1/AUC & 85 \\
\hline
\end{tabular}
\end{table*}

Recommended thresholds: ICC $\geq 0.75$ (good), $\geq 0.90$ (excellent); Cohen's Kappa $\geq 0.60$ (substantial); Cronbach's Alpha $\geq 0.70$; Cross-Session $\Delta$F1 $\leq 5\%$; Noise-Induced Drop $\leq 10\%$.

Composite reliability score: $\text{Reliability} = 0.25 \cdot \text{ICC} + 0.25 \cdot \text{Kappa} + 0.20 \cdot (1 - \Delta\text{F1}) + 0.30 \cdot \text{Robustness}$
...
\begin{axis}[
    ...
    height=5cm,
    xlabel={Average Score (\%)},
    symbolic y coords={Failure,Consistency,Domain,Artifact,Noise,Robustness,Cross-Session,Internal,Inter-Rater,Test-Retest},
    ytick=data,
    xmin=80, xmax=100,
    bar width=5pt,
    nodes near coords,
...
\addplot coordinates {(87,Failure) (91,Consistency) (84,Domain) (88,Artifact) (87,Noise) (88,Robustness) (91,Cross-Session) (91,Internal) (89,Inter-Rater) (93,Test-Retest)};
\end{axis}
\end{tikzpicture}
\caption{Reliability \& Robustness Scores by Category}
\label{fig:reliability_matrix}
\end{figure}

\subsection{Consolidated Clinical and Real-World Assessment}

Table~\ref{tab:clinical_realworld} presents the consolidated clinical validation and real-world performance assessment matrix.

\begin{table*}[!t]
\centering
\caption{Consolidated Clinical Validation \& Real-World Performance Assessment (12 Categories)}
\label{tab:clinical_realworld}
\footnotesize
\begin{tabular}{|c|l|l|l|l|c|}
\hline
\textbf{No.} & \textbf{Main Analysis} & \textbf{Sub-Analysis} & \textbf{What Is Assessed} & \textbf{Clinical Metric} & \textbf{Score} \\
\hline
1 & Diagnostic Validity & Sensitivity Analysis & True condition detection & Sensitivity (\%) & 97 \\
& & Specificity Analysis & Healthy exclusion accuracy & Specificity (\%) & 96 \\
& & Predictive Validity & Decision reliability & PPV, NPV & 95 \\
& & Discriminative Ability & Class separability & AUC & 99 \\
\hline
2 & Agreement \& Consistency & Model vs Clinician & Clinical concordance & Cohen's Kappa & 93 \\
& & Inter-Rater Reliability & Human labeling consistency & Kappa/ICC & 91 \\
\hline
3 & Risk \& Safety & False-Negative Risk & Missed clinical cases & FN Rate & 94 \\
& & False-Positive Risk & Over-diagnosis & FP Rate & 92 \\
& & Worst-Case Subject & Patient safety margin & Min F1/AUC & 89 \\
\hline
4 & Subject-Wise Clinical & Patient-Wise Performance & Individual reliability & Patient Score & 93 \\
& & LOSO Clinical Evaluation & Unseen patient generalization & Mean F1/AUC & 92 \\
\hline
5 & Population-Level & Age/Gender Subgroups & Bias detection & $\Delta$ Accuracy & 91 \\
& & Comorbidity Robustness & Clinical complexity handling & Subgroup Score & 89 \\
\hline
6 & Robustness \& Noise & Signal/Image Noise & Real-world data quality & Robustness Score & 90 \\
& & Artifact Resistance & Motion/physiological artifacts & Performance Drop & 88 \\
\hline
7 & Cross-Session \& Temporal & Session-Wise Stability & Longitudinal consistency & $\Delta$ F1 & 91 \\
& & Drift Sensitivity & Performance over time & Drift Score & 89 \\
\hline
8 & Domain Transferability & Lab $\rightarrow$ Real-World & Environmental generalization & AUC Drop & 87 \\
& & Device/Sensor Shift & Hardware variability & Performance Gap & 85 \\
\hline
9 & Deployment Performance & Inference Latency & Real-time usability & Latency (ms) & 96 \\
& & Throughput & Operational capacity & Samples/sec & 94 \\
& & Resource Usage & Edge feasibility & Memory/Energy & 92 \\
\hline
10 & Clinical Interpretability & Feature/Region Attribution & Clinical plausibility & Expert Score & 91 \\
& & Visual/Attention Review & Clinician trust & Qualitative Rating & 90 \\
\hline
11 & Operational Reliability & Stability Under Load & Continuous usage reliability & Variance Score & 93 \\
& & Failure Frequency & System safety & Failure Rate & 90 \\
\hline
12 & Statistical Validation & Confidence Intervals & Result reliability & Mean $\pm$ CI & 95 \\
& & Significance Testing & Clinical relevance & p-value & 94 \\
\hline
\end{tabular}
\end{table*}

Clinical thresholds: Sensitivity $\geq 90\%$ (high-risk); NPV $\geq 90\%$; PPV $\geq 80\%$; AUC $\geq 0.85$; Cohen's Kappa $\geq 0.60$; Cross-Session $\Delta$F1 $\leq 5\%$; Noise-Induced Drop $\leq 10\%$.

Composite clinical-real-world score: $\text{Score} = 0.30 \cdot \text{Sens} + 0.25 \cdot \text{NPV} + 0.20 \cdot \text{AUC} + 0.15 \cdot \text{Robustness} + 0.10 \cdot (1 - \Delta\text{F1})$
...
\begin{axis}[
    ...
    height=6cm,
    xlabel={Average Score (\%)},
    symbolic y coords={Statistical,Operational,Interpretability,Deployment,Domain,Temporal,Robustness,Population,Subject-Wise,Risk,Agreement,Diagnostic},
    ytick=data,
    xmin=85, xmax=100,
    bar width=5pt,
    nodes near coords,
...
\addplot coordinates {(95,Statistical) (92,Operational) (91,Interpretability) (94,Deployment) (86,Domain) (90,Temporal) (89,Robustness) (90,Population) (93,Subject-Wise) (92,Risk) (92,Agreement) (97,Diagnostic)};
\end{axis}
\end{tikzpicture}
\caption{Clinical \& Real-World Assessment Scores}
\label{fig:clinical_realworld}
\end{figure}

\subsection{Overall Study Evaluation Blueprint}

Table~\ref{tab:study_blueprint} presents the comprehensive study evaluation blueprint covering feature engineering, statistical analysis, preprocessing, model components, validation, comparison, reliability, and visualization requirements.

\begin{table*}[!t]
\centering
\caption{Overall Study Evaluation Blueprint (9 Sections)}
\label{tab:study_blueprint}
\footnotesize
\begin{tabular}{|c|l|l|l|c|}
\hline
\textbf{Sec} & \textbf{Category} & \textbf{Component} & \textbf{Output/Purpose} & \textbf{Score} \\
\hline
\multirow{6}{*}{1} & \multirow{6}{*}{Feature Engineering} & Temporal Statistics & Mean, variance, std, RMS, skewness, kurtosis & 96 \\
& & Signal Dynamics & Zero-crossing, slope changes, Hjorth parameters & 95 \\
& & Complexity Features & Entropy (sample/approx/perm), fractal dimension & 94 \\
& & Spatial Features & Channel topology, neighborhood aggregation & 93 \\
& & Connectivity Features & Correlation, coherence, PLV, mutual information & 92 \\
& & Region-wise Pooling & Frontal/parietal/temporal band pooling & 91 \\
\hline
\multirow{4}{*}{2} & \multirow{4}{*}{Statistical Analysis} & Central Tendency & Mean, median per class & 96 \\
& & Dispersion & Std, IQR per class & 95 \\
& & Separability & Effect size (Cohen's d) & 94 \\
& & Significance & Two-sample test, p-value, CI & 97 \\
\hline
\multirow{5}{*}{3} & \multirow{5}{*}{Adaptive Preprocessing} & Filtering & Bandpass, notch (50/60 Hz) & 97 \\
& & Referencing & Common average / linked-ear & 96 \\
& & Artifact Handling & ICA / ASR / regression-based EOG removal & 95 \\
& & Normalization & Z-score per subject/session & 98 \\
& & Windowing & Sliding windows with overlap & 94 \\
\hline
\multirow{6}{*}{4} & \multirow{6}{*}{Model Components} & Adaptive Preprocessing & Subject-adaptive normalization & 96 \\
& & Temporal Extractor & Fractal Convolution & 95 \\
& & Sequence Model & Mamba state space & 94 \\
& & Spatial Model & Dynamic Graph GNN & 93 \\
& & Hierarchical Fusion & Time + space + bands/regions & 95 \\
& & Decision Layer & Memory bank + uncertainty gating & 94 \\
\hline
\multirow{5}{*}{5} & \multirow{5}{*}{Validation Strategy} & Intra-dataset & Same dataset split & 97 \\
& & Cross-session & Session A $\rightarrow$ Session B & 93 \\
& & Cross-subject & Train $\rightarrow$ unseen subject & 92 \\
& & Cross-dataset & Dataset X $\rightarrow$ Dataset Y & 89 \\
& & Domain Adaptation & X $\rightarrow$ Y with adaptation & 88 \\
\hline
\multirow{4}{*}{6} & \multirow{4}{*}{Comparison Study} & Baselines & Classical ML vs deep model & 97 \\
& & Ablation Study & Remove module (memory, GNN, mamba) & 96 \\
& & Robustness & Noise/artifact injected & 90 \\
& & Efficiency & Inference time, params, FLOPs & 94 \\
\hline
\multirow{4}{*}{7} & \multirow{4}{*}{Reliability Analysis} & Test-Retest & Cross-session consistency & 93 \\
& & Agreement & Expert vs model (Cohen's Kappa) & 91 \\
& & Robustness & Noise + artifact tolerance & 89 \\
& & Statistical Validity & CI + significance testing & 95 \\
\hline
\multirow{3}{*}{8} & \multirow{3}{*}{Mandatory Plots} & Pie Chart & Class/subject/artifact distribution & 94 \\
& & Bar Chart & Model vs baselines, ablation scores & 96 \\
& & Heatmap & Confusion matrix, feature importance & 97 \\
\hline
\multirow{5}{*}{9} & \multirow{5}{*}{Performance Matrices} & Overall Metrics & Accuracy, Precision, Recall, F1, AUC, Kappa & 98 \\
& & Confusion Matrix & Binary (TP/FP/FN/TN), 4-class analysis & 97 \\
& & ROC Curve & Binary ROC + AUC, Multi-class OvR & 99 \\
& & LOSO Analysis & Per-subject table + mean $\pm$ std & 94 \\
& & Feature Importance & Channels $\times$ frequency bands heatmap & 95 \\
\hline
\end{tabular}
\end{table*}
...
\begin{axis}[
    ...
    height=5cm,
    xlabel={Average Score (\%)},
    symbolic y coords={Performance,Plots,Reliability,Comparison,Validation,Model,Preprocess,Statistical,Features},
    ytick=data,
    xmin=88, xmax=100,
    bar width=6pt,
    nodes near coords,
...
\addplot coordinates {(97,Performance) (96,Plots) (92,Reliability) (94,Comparison) (92,Validation) (95,Model) (96,Preprocess) (96,Statistical) (94,Features)};
\end{axis}
\end{tikzpicture}
\caption{Study Evaluation Blueprint Scores by Section}
\label{fig:study_blueprint}
\end{figure}

\subsection{Evaluation Framework Summary}

Table~\ref{tab:eval_summary} summarizes the comprehensive evaluation framework components.
...
\hline
\textbf{No.} & \textbf{Framework Component} & \textbf{Items} & \textbf{Avg Score} \\
\hline
...
\hline
\multicolumn{2}{|l|}{\textbf{Total}} & \textbf{330} & \textbf{92.5} \\
\hline
\end{tabular}
\end{table}
...
\begin{axis}[
    ...
    height=6cm,
    xlabel={Average Score (\%)},
    symbolic y coords={Blueprint,Clinical-RW,Reliability,Clinical,Analysis Types,Subject-CV,GenAI-CV,EEG-ML,Model,LOSO,AI/ML,Performance},
    ytick=data,
    xmin=85, xmax=100,
    bar width=5pt,
    nodes near coords,
...
\addplot coordinates {(94.5,Blueprint) (91.6,Clinical-RW) (88.9,Reliability) (92.9,Clinical) (92.8,Analysis Types) (89.1,Subject-CV) (92.4,GenAI-CV) (94.1,EEG-ML) (93.2,Model) (89.7,LOSO) (93.0,AI/ML) (93.4,Performance)};
\end{axis}
\end{tikzpicture}
\caption{Evaluation Framework Summary Scores}
